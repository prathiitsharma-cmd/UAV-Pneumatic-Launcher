...
125 mm and why the 1.5x force safety factor matters as much as it does.
...
| 6 | Safety factors (1.5x / 1.25x) | Standard-derived rather than assumed, so treated as fixed. Would move final sizes roughly linearly, but isn't really a "what if we were wrong" case the way the top items are. |
